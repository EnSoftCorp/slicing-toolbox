...
public class FOWControlDependenceGraph extends DependenceGraph {
	...
	public FOWControlDependenceGraph(Graph cfg, boolean purgeAugmentations){
		...
		if(purgeAugmentations){
			...
			AtlasHashSet<Edge> augmentationEdgesToRemove = new AtlasHashSet<Edge>(Query.universe().edges(FOWControlDependenceGraph.AUGMENTATION_EDGE).eval().edges());
			...
			AtlasHashSet<Node> augmentationNodesToRemove = new AtlasHashSet<Node>(Query.universe().nodes(FOWControlDependenceGraph.AUGMENTATION_NODE).eval().nodes());
